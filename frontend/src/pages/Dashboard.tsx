...
import { DollarSign, TrendingUp, Calendar, MapPin, PiggyBank, Fuel, Navigation, AlertTriangle, RefreshCw, Receipt } from 'lucide-react';
import { useToast } from '../components/Toast';
import { Skeleton, useCountUp, PageHeader } from '../components/ui';
...
const donutPalette = ['#f59e0b', '#ef4444', '#8b5cf6', '#22d3ee', '#f43f5e', '#a3e635', '#38bdf8', '#fb923c', '#a1a1aa', '#2dd4bf'];
...
export default function Dashboard() {
  ...
  const fmtKm = (v: number) => v.toFixed(0) + ' km';
  ...
  const totalExpenses = expenses.reduce((a, c) => a + c.total, 0);
  const donutData = {
    labels: expenses.map(c => catLabels[c.category] || c.category),
    datasets: [{
      data: expenses.map(c => c.total),
      backgroundColor: expenses.map((_, i) => donutPalette[i % donutPalette.length]),
      borderColor: '#09090b',
      borderWidth: 3,
      hoverOffset: 6,
    }],
  ...
  const donutOptions = {
    ...
    cutout: '68%',
    plugins: {
      legend: { position: 'bottom' as const, labels: { color: '#a1a1aa', usePointStyle: true, pointStyleWidth: 8, boxHeight: 6, padding: 14, font: { size: 11 } } },
      tooltip: { backgroundColor: '#18181b', borderColor: 'rgba(255,255,255,0.1)', borderWidth: 1, padding: 10, cornerRadius: 10, callbacks: { label: (c: any) => ' ' + c.label + ': R$ ' + Number(c.parsed).toFixed(2).replace('.', ',') } },
    },
  };
  ...
  return (
    <div className="space-y-6 animate-slide-up">
      <PageHeader
        ...
        right={<span className="chip"><span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" /> Online</span>}
      />
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 stagger">
        <StatCard icon={DollarSign} label="Faturamento Hoje" value={data.today_revenue} format={fmt} gradient="from-amber-400 to-orange-500" />
        <StatCard icon={TrendingUp} label="Faturamento Semana" value={data.week_revenue} format={fmt} gradient="from-emerald-400 to-teal-500" />
        <StatCard icon={Calendar} label="Faturamento Mes" value={data.month_revenue} format={fmt} gradient="from-blue-400 to-indigo-500" />
        <StatCard icon={MapPin} label="KM Rodados Hoje" value={data.today_km} format={fmtKm} gradient="from-violet-400 to-purple-500" />
        <StatCard icon={PiggyBank} label="Lucro Liquido" value={data.net_profit} format={fmt} gradient="from-emerald-400 to-green-500" />
        <StatCard icon={Fuel} label="Combustivel Total" value={data.total_fuel} format={fmt} gradient="from-red-400 to-rose-500" />
      </div>
        ...
        <div className="card card-hover p-5 lg:col-span-2">
          ...
          <div className="h-56">
            {Array.isArray(data.daily_revenue) && data.daily_revenue.length > 0 ? (
              ...
              <div className="flex items-center justify-center h-full"><p className="text-zinc-500 text-sm">Nenhum dado de receita ainda</p></div>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 stagger">
        <div className="card card-hover p-5 lg:col-span-1">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-medium text-zinc-100">Despesas por Categoria</h2>
            <span className="chip"><Receipt size={12} /> {fmt(totalExpenses)}</span>
          </div>
          <div className="h-64">
            {expenses.length > 0 ? (
              <Doughnut data={donutData} options={donutOptions} />
            ) : (
              <div className="flex items-center justify-center h-full"><p className="text-zinc-500 text-sm">Sem despesas neste mes</p></div>
            )}
          </div>
        </div>
        ...
        <div className="card card-hover p-5 lg:col-span-2">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <Row label="KM Rodados (hoje)" value={fmtKm(data.today_km)} />
            <Row label="Progresso da Meta" value={pct.toFixed(0) + '%'} />
          </div>
          <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4">
            <Row label="Faturamento Hoje" value={fmt(data.today_revenue)} />
            <Row label="Faturamento Semana" value={fmt(data.week_revenue)} />
            <Row label="Faturamento Mes" value={fmt(data.month_revenue)} />
            <Row label="Combustivel Total" value={fmt(data.total_fuel)} />
          </div>
        </div>
      </div>
    </div>
...
function StatCard({ icon: Icon, label, value, format, gradient }: { icon: any; label: string; value: number; format: (n: number) => string; gradient: string }) {
  ...
  return (
    <div className="card card-hover p-4 group">
      ...
      <p className="text-lg font-semibold text-zinc-100">{format(animated)}</p>
    </div>
  );
}
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Skeleton className="h-40 lg:col-span-2 rounded-2xl" />
      </div>
    </div>
  );
}
